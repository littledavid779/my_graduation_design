...
void run_test(int num_thread, string load_data, string run_data,
  ...
  *************************************************
  ...
  printf("LatencyA:");
  vector<string> percent = {"avg", "0", "50", "90", "99", "99.9", "100"};
  vector<double> percent_val = {-1, 0, 0.5, 0.9, 0.99, 0.999, 1};
  for (size_t i = 0; i < percent.size(); i++) {
    printf("%8s,", percent[i].c_str());
  }
  printf("\nLatencyB:" KYEL_ITA_BOLD);
  for (size_t i = 0; i < percent.size(); i++) {
    if (i == 0) {
      printf("%8lu,", avg);
    } else if (i == percent.size() - 1) {
      printf("%8lu", latency_all[sz - 1]);
    } else {
      printf("%8lu,", latency_all[size_t(percent_val[i] * sz)]);
    }
  }
  printf("\n" KRESET);
#endif
...

  {
    ...
    sw.start();
    // measure.start();
    // auto buffer_root = tree->current->root.load();
    // snapshot::shot(buffer_root);
    // printf("[snapshot measure] buffer tree shot time: %.6f ms\n",
    //        measure.elapsed<std::chrono::milliseconds>());
    // measure.start();
    NodeBase *root = device_tree->root.load();
    btree_snapshot::shot(root);
    // printf("[snapshot measure] device tree shot time: %.6f ms\n",
    //        measure.elapsed<std::chrono::milliseconds>());
    auto shot_time = sw.elapsed<std::chrono::milliseconds>();
    printf("[snapshot] total shot time: %.6f ms\n", shot_time);
  }
  delete tree;
  para->Close();
...
void recover() {
  ...
  Timer measure;
  measure.start();
  auto [rt, timestamp] = btree_snapshot::recover();
  printf("[snapshot measure] recover time: %.6f ms\n",
         measure.elapsed<std::chrono::milliseconds>());
  measure.start();
  auto kv = btree_snapshot::recover_from_wal(timestamp);
  printf("[snapshot measure] recover_from_wal time: %.6f ms\n",
         measure.elapsed<std::chrono::milliseconds>());
  measure.start();
  ZoneManagerPool *para =
      new ZoneManagerPool(MAX_CACHED_PAGES_PER_ZONE, MAX_NUMS_ZONE, ZNS_DEVICE);
  BTree *device_tree = new BTree(para);
  device_tree->root.store(rt);
  btreeolc::ZBTree<KeyType, ValueType> *tree =
      new btreeolc::ZBTree<KeyType, ValueType>(device_tree);
  printf("[snapshot measure] tree init time: %.6f ms\n",
         measure.elapsed<std::chrono::milliseconds>());
  auto insert = [&](size_t start, size_t len, int tid) {
    auto end = start + len;
    // cout << "start:" << start << "end:" << start + len << endl;
    for (size_t i = start; i < end; i++) {
      tree->Insert(kv[i].first, kv[i].second);
    }
  };
  int load_thread = 48;
  int sz = kv.size();
  auto part = sz / load_thread;
  auto left = sz % load_thread;
  vector<thread> ths(load_thread);
  ...
  for (size_t i = 0; i < left; i++) {
    tree->Insert(kv[LOAD_SIZE - left + i].first,
                 kv[LOAD_SIZE - left + i].second);
  }
  for (size_t i = 0; i < load_thread; i++) {
    ths[i].join();
  }
  printf("[snapshot measure] load time: %.6f ms\n",
         measure.elapsed<std::chrono::milliseconds>());
  // tree->FlushAll();
  // para->FlushAllPages();
  auto recover_time = sw.elapsed<std::chrono::milliseconds>();
  printf("[snapshot] recover size: %ld\n", kv.size());
  printf("[snapshot] recover time: %.6f ms\n", recover_time);
  delete tree;
  delete para;
}

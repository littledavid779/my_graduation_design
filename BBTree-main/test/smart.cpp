...
int main(int argc, char* argv[]) {
  ...
  cmd.nsid = 0xffffffff;
  cmd.addr = (__u64)data;
  cmd.data_len = SMART_LEN;
  cmd.cdw10 = 0x007F0002;  // bit 31:28 reserved = 0h
  ...
  if (data[0] & 0x2) printf("–Temperature has exceed a critical threshold\n");
  if (data[0] & 0x4) printf("–Device Reliability has been degraded\n");
  if (data[0] & 0x8) printf("–Volatile memory backup device has failed\n");
  if (*((__u16*)(data + 1))) {
